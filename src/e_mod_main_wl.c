...
static void
_e_keyrouter_remove_client_from_list(E_Client *ec)
{
   int index = 0;
   CHECK_NULL(ec);
   ...
   for(index=0; index<MAX_HWKEYS; index++)
     {
        ...
        /* exclusive grab */
        if ((krt->HardKeys[index].excl_ptr) && (ec == krt->HardKeys[index].excl_ptr->ec))
          {
...
_e_keyrouter_client_cb_remove(void *data, int type, void *event)
{
   ...
   _e_keyrouter_remove_client_from_list(ec);
