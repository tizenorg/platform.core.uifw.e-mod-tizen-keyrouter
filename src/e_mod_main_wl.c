...
static Eina_Bool _e_keyrouter_util_do_privilege_check(struct wl_client *client, struct wl_resource *surface, int socket_fd, uint32_t mode, uint32_t keycode);
...
_e_keyrouter_keygrab_set(struct wl_client *client, struct wl_resource *surface, uint32_t key, uint32_t mode)
{
...
#ifdef ENABLE_CYNARA
   if (EINA_FALSE == _e_keyrouter_util_do_privilege_check(client, surface,
   ...
   if (TIZEN_KEYROUTER_MODE_DISPLAYOFF < mode)
     {
...
static void
_e_keyrouter_cb_get_register_delivery(struct wl_client *client, struct wl_resource *resource,uint32_t key)
{
   (void) client;

   tizen_keyrouter_send_get_register_delivery_notify(resource, key, krt->isRegisterDelivery);
}
...
static const struct tizen_keyrouter_interface _e_keyrouter_implementation = {
   ...
   _e_keyrouter_cb_get_register_delivery
};
...
_e_keyrouter_query_tizen_key_table(void)
{
   ...
   krt->display_key_filter_mode = kconf->display_key_filter_mode;
   ...
   EINA_LIST_FOREACH(kconf->KeyList, l, data)
     {
        ...
        krt->HardKeys[data->keycode].display_filter = data->display_filter ? EINA_TRUE : EINA_FALSE;
     ...
     switch (state)
       {
          case DISPLAY_STATE_SCREEN_OFF:
             krt->display_off = EINA_TRUE;
             break;
          case DISPLAY_STATE_NORMAL:
             krt->display_off = EINA_FALSE;
             break;
          /* FIXME: Check a state change is possible
           *        screen off to screen dim state
           */
...
static Eina_Bool
_e_keyrouter_util_do_privilege_check(struct wl_client *client, struct wl_resource *surface,  int socket_fd, uint32_t mode, uint32_t keycode)
{
