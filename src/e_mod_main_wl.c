...
_e_keyrouter_init(E_Module *m)
{
   ...
   if (!(kconfig = E_NEW(E_Keyrouter_Config_Data, 1)))
     {
        KLDBG("Failed to allocate config data for keyrouter !\n");
        goto err;
     }
...
_e_keyrouter_query_tizen_key_table(void)
{
   ...
   krt->HardKeys = E_NEW(E_Keyrouter_Grabbed_Key, kconf->max_keycode + KEYCAP + 1);

   krt->numTizenHWKeys = kconf->num_keycode;
   krt->maxTizenHWKeys = kconf->max_keycode + KEYCAP;
   ...
   EINA_LIST_FOREACH(kconf->KeyList, l, data)
     {
        ...
        krt->HardKeys[data->keycode + KEYCAP].keycode = data->keycode + KEYCAP;
        krt->HardKeys[data->keycode + KEYCAP].keyname = eina_stringshare_add(data->name);
     }
}
