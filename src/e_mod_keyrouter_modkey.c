...
{
   KLERR("[jeon] modkey timer is called.\n");
...
{
   KLERR("[jeon] stepkey timer is called.\n");
   ...
   EINA_LIST_FOREACH(krt->modkey_list, l, data)
     {
        for (i = 0; i < data->num_keys; i++)
          {
             KLERR("[jeon] ev->keycode: %d, keycodes[%d]: %d, data->pressed: 0x%x\n", ev->keycode, i, data->keycode[i], data->pressed);
             if (ev->keycode == data->keycode[i])
               {
                  data->pressed |= (1 << i);
                  KLERR("[jeon] PRESSED! data->pressed: 0x%x, max: 0x%x\n", data->pressed, _calc_pressed_keys(data->num_keys));
                  if (krt->modkey_timer) ecore_timer_del(krt->modkey_timer);
...
e_keyrouter_stepkey_check(Ecore_Event_Key *ev)
{
   ...
   EINA_LIST_FOREACH(krt->stepkey_list, l, data)
     {
        KLERR("[jeon] ev->keycode: %d, data->keycode[%d]: %d\n", ev->keycode, data->idx, data->keycode[data->idx]);
...
_e_keyrouter_modkey_mod_init(Eina_List *list_keys, int num_keys, Eina_Bool press_only)
{
   ...
   KLERR("[jeon] num_keys: %d, press_only: %d\n", num_keys, press_only);
...
_e_keyrouter_modkey_step_init(Eina_List *list_keys, int num_keys, int action)
{
   ...
   KLERR("[jeon] num_keys: %d, action: %d\n", num_keys, action);
...
e_keyrouter_modkey_init(void)
{
   ...
   EINA_LIST_FOREACH(kconf->ModifierList, l, mdata)
     {
        EINA_LIST_FOREACH(mdata->ModKeys, ll, ndata)
          {
             KLERR("[jeon] mod keys: %s\n", ndata->name);
          }
        KLERR("[jeon] num_modkeys: %d, combination: %d, press_only: %d\n", mdata->num_modkeys, mdata->combination, mdata->press_only);
